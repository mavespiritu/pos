...
/**
 * This is the model class for table "accounting_income_freebies_and_icons".
 *
 * @property int $id
 ...
 */
class FreebieAndIcon extends \yii\db\ActiveRecord
{
    public $seasons_id;
    public $frequency_id;
    public $page_id;
    ...
    {
        return 'accounting_income_freebies_and_icons';
    }
    ...
    {
        return [
            [['seasons_id', 'frequency_id', 'date_id', 'page_id'], 'required', 'on' => 'searchFreebieAndIcon'],
            [['season_id', 'student_id', 'code_id', 'pr', 'ar_no', 'amount'], 'required'],
            [['season_id', 'code_id', 'student_id'], 'integer'],
            [['amount'], 'number'],
            [['pr', 'ar_no'], 'string', 'max' => 250],
    ...
    {
        return $this->hasOne(Income::className(), ['income_id' => 'id'])->where(['accounting_income.income_type_id' => '2']);
    }

    public function getAmountType()
    {
        $income = $this->getIncome()->where(['income_type_id' => '2'])->one();
