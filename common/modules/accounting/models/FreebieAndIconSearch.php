...
/**
 * FreebieAndIconSearch represents the model behind the search form of `common\modules\accounting\models\FreebieAndIcon`.
 */
class FreebieAndIconSearch extends FreebieAndIcon
{
    public $seasonName;
    public $codeName;
    public $studentName;
    ...
    public function rules()
    {
        return [
            ...
            [['pr', 'ar_no', 'seasonName', 'codeName', 'studentName', 'amountType', 'datetime', 'transactionNumber'], 'safe'],
            [['amount'], 'number'],
        ];
    }
    ...
    public function search($params)
    {
        ...
        if(in_array('TopManagement',$rolenames)){
            $query = $access ? $access->branch_program_id!= '' ? FreebieAndIcon::find()
                ->joinWith('income')
                ->joinWith('season')
                ...
                ->joinWith('code')
                ->joinWith('student')
                ->andWhere(['accounting_income.income_type_id' => '2'])
                ->andWhere(['not in', 'accounting_income_freebies_and_icons.season_id', $archivedSeasons])
