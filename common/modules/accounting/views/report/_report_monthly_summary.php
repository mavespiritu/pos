...
<table class="table table-bordered table-condensed table-striped table-hovered table-responsive" style="width: 100%;">
	<thead>
		<tr>
			...
			<?php foreach($cutoffs as $cutoff){ ?>
				<th style="width: <?= ((count($cutoffs)+3)/100)*100 ?>%">
	...
	<tbody>
		...
		<tr>
			...
			<?php foreach($cutoffs as $cutoff){ ?>
				<td align=right style="width: 10%;"><?= '<b>'.number_format($total['income'][$cutoff['start'].' - '.$cutoff['end']], 2).'</b>' ?></td>
			<?php } ?>
			<td align=right style="width: 10%;"><?= '<b>'.number_format($incomeEnrolmentTotal + $freebieTotal, 2).'</b>' ?></td>
		</tr>
		<tr>
