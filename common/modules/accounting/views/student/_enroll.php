...
<div class="student-form">
    ...
    <?php $form = ActiveForm::begin(['id' => 'enrolment-form']); ?>
    <h3><?= $model->id_number.' - '.$model->fullName ?>
        <small class="pull-right">
            <?= empty($transferee) ? Html::button('Transfer The Student', ['value' => Url::to(['/accounting/student/transfer', 'id' => $model->id, 'branch_id' => $season->branchProgram->branch->id, 'season_id' => $season->id]), 'id' => 'transfer-button', 'class' => 'btn btn-warning']) : '' ?>
            <?= !empty($tuition) && empty($dropout) ? Html::button('Drop The Student', ['value' => Url::to(['/accounting/student/dropout', 'id' => $model->id, 'season_id' => $season->id]), 'id' => 'drop-button', 'class' => 'btn btn-danger']) : '' ?>

            <?= !empty($transferred) ? Html::button('Undo Transfer of Student', ['value' => Url::to(['/accounting/student/untransfer', 'id' => $model->id, 'branch_id' => $season->branchProgram->branch->id, 'season_id' => $season->id]), 'id' => 'untransfer-button', 'class' => 'btn btn-success']) : '' ?>
            <?= !empty($tuition) && !empty($dropout) ? Html::button('Remove From Dropouts', ['value' => Url::to(['/accounting/student/remove-dropout', 'id' => $model->id, 'season_id' => $season->id]), 'id' => 'remove-drop-button', 'class' => 'btn btn-success']) : '' ?>
        </small>
    </h3><hr>
    <?php if(!empty($dropout)){ ?>
        <p class="text text-danger"><i class="glyphicon glyphicon-info-sign"></i> Student is dropped.</p>
        <p><b>Reasons For Dropping: </b><br><?= Html::encode($dropout['reason']) ?></p>
        <p><b>Date Dropped: </b><br><?= $dropout['drop_date'] ?></p>
        <p><b>Authorized By: </b><br><?= Html::encode($dropout['authorized_by']) ?></p>
    <?php }else{ ?>
         <?= !empty($transferee) ? '<p class="text text-danger">Student is transferred to '.$transfereeModel->toSeason->seasonName.'. </p>' : '' ?>
        <div class="row">
            <div class="col-md-12">
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <?= $form->field($incomeEnrolmentModel, 'or_no')->textInput(['maxlength' => true, 'value' => $incomeEnrolmentModel->or_no!='' ? $incomeEnrolmentModel->or_no : $current_or, 'disabled' => 'disabled']) ?> 
                            </div>
                            <div class="col-md-12">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <?php if (empty($transferee)) {  ?>
                                        <?php if (!$incomeEnrolmentModel) {  ?>
                                            <?= Html::submitButton('Update', ['class' => 'btn btn-block btn-success', 'id' => 'submit-button']) ?>
                                        <?php }else{ ?>
                                            <?php if($orStatus == 1) { ?>
                                                <?= Html::submitButton('Save', ['class' => 'btn btn-block btn-success', 'id' => 'submit-button']) ?>
                                            <?php }else{ ?>
                    ...
                    <div class="col-md-8">
                        <div class="row">
                            <div class="col-md-6">
                                <div id="balance-information"></div>
                            </div>
                            <div class="col-md-6">
            ...
            $( document ).ready(function() {
                /*$("#enrolment-form").on("beforeSubmit", function(e) {
                  ...
                  $.ajax({
                    ...
                    alert("Enrolment has been saved.");
                ...
                $("#advance-enrolment-button").click(function(){
                  $("#genericModal").modal("show").find("#genericModalContent").load($(this).attr("value"));
                });
                ...
                $("#remove-drop-button").click(function(){
                  $("#genericModal").modal("show").find("#genericModalContent").load($(this).attr("value"));
                });
                ...
                $("#untransfer-button").click(function(){
                  $("#genericModal").modal("show").find("#genericModalContent").load($(this).attr("value"));
                });
                ...
                $.ajax({
                    url: "'.Url::to(['/accounting/student/show-balance']).'?id=" + $("#student_id").val() + "&season_id=" + $("#student-season_id").val(),
                    success: function (data) { 
                        $("#balance-information").empty();
                        $("#balance-information").hide();
                        $("#balance-information").fadeIn();
                        $("#balance-information").html(data);

                    }
                });

                $.ajax({
                    url: "'.Url::to(['/accounting/student/show-season']).'?id=" + $("#student-season_id").val(),
                    success: function (data) { 
                ...
                $.ajax({
                    url: "'.Url::to(['/accounting/student/show-advance-enrolment-table']).'?id=" + $("#student_id").val(),
                    success: function (data) { 
                        $("#advance-enrolments-table").empty();
                        $("#advance-enrolments-table").hide();
                        $("#advance-enrolments-table").fadeIn();
                        $("#advance-enrolments-table").html(data);
