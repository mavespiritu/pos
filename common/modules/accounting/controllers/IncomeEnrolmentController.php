...
use common\modules\accounting\models\IncomeEnrolmentSearch;
...
use kartik\mpdf\Pdf;
/**
 * IncomeEnrolmentController implements the CRUD actions for IncomeEnrolment model.
 */
class IncomeEnrolmentController extends Controller
{
    /**
     * {@inheritdoc}
     */
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index'],
                        'allow' => true,
                        'roles' => ['manageIncomeEnrolment'],
                    ],
                    [
                        'actions' => ['update'],
                        'allow' => true,
                        'roles' => ['updateIncomeEnrolment'],
                    ],
                    [
                        'actions' => ['delete'],
                        'allow' => true,
                        'roles' => ['deleteIncomeEnrolment'],
                    ],
                ],
            ],
        ];
    ...
    {
        $model = new IncomeEnrolment();
        $model->scenario = 'searchIncomeEnrolment';
        ...
        {
            $postData = Yii::$app->request->post()['IncomeEnrolment'];
            ...
            if($season)
            {
                ...
                if($postData['frequency_id']!="" && $postData['date_id']!="")
                {
                    ...
                    {
                        $data = $access ? $access->branch_program_id!= '' ? IncomeEnrolment::find()
                                ->select([
                                    ...
                                    'accounting_income_enrolment.ar_no',
                    ...
                    $content = $this->renderPartial('_income-enrolment-report', [
                        'data' => $data,
                        'season' => $season,
                        'postData' => $postData,
                        'dates' => $dates,
                    ...
                    'filename' => $postData['frequency_id'].' Report: Enrolments Income - '.$season->seasonName.' - Page '.($postData['page_id']+1).' of '.$pages.'.pdf',
                    'content' => $content,  
                    'marginLeft' => 11.4,
                    'marginRight' => 11.4,
                    ...
                    'options' => ['title' => 'Enrolment Income'],
                    'methods' => [ 
                        'SetHeader'=>[$postData['frequency_id'].' Report: Enrolments Income - '.$season->seasonName.' - Page '.($postData['page_id']+1).' of '.$pages], 
        ...
        return $this->renderAjax('_search-income-enrolment',[
            'model' => $model,
            'seasons' => $seasons,
    ...
    public function actionPageList($season_id = '', $frequency_id = '', $date_id = '') {
        ...
        if($season)
        {
            ...
            if($frequency_id != '' && $date_id != '')
            {
                ...
                {
                    $count = $access ? $access->branch_program_id!= '' ? IncomeEnrolment::find()
                            ->leftJoin('accounting_income', 'accounting_income.income_id = accounting_income_enrolment.id and accounting_income.income_type_id = 1')
                            ->leftJoin('accounting_season','accounting_season.id = accounting_income_enrolment.season_id')
                            ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
                            ->andWhere(['like','accounting_income.datetime', $dates['year']])
    ...
    /**
     * Lists all IncomeEnrolment models.
     * @return mixed
     */
    public function actionIndex()
    {
        ...
        $searchModel = new IncomeEnrolmentSearch();
        ...
        if(in_array('TopManagement',$rolenames)){
            $overall = $access ? $access->branch_program_id!= '' ? IncomeEnrolment::find()
            ...
            $overallCash = $access ? $access->branch_program_id!= '' ? IncomeEnrolment::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_income','accounting_income.income_id = accounting_income_enrolment.id and accounting_income.income_type_id = 1')
                        ->leftJoin('accounting_season', 'accounting_season.id = accounting_income_enrolment.season_id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallCheck = $access ? $access->branch_program_id!= '' ? IncomeEnrolment::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_income','accounting_income.income_id = accounting_income_enrolment.id and accounting_income.income_type_id = 1')
                        ->leftJoin('accounting_season', 'accounting_season.id = accounting_income_enrolment.season_id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallToday = $access ? $access->branch_program_id!= '' ? IncomeEnrolment::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_income','accounting_income.income_id = accounting_income_enrolment.id and accounting_income.income_type_id = 1')
                        ->leftJoin('accounting_season', 'accounting_season.id = accounting_income_enrolment.season_id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallCashToday = $access ? $access->branch_program_id!= '' ? IncomeEnrolment::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_income','accounting_income.income_id = accounting_income_enrolment.id and accounting_income.income_type_id = 1')
                        ->leftJoin('accounting_season', 'accounting_season.id = accounting_income_enrolment.season_id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallCheckToday = $access ? $access->branch_program_id!= '' ? IncomeEnrolment::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_income','accounting_income.income_id = accounting_income_enrolment.id and accounting_income.income_type_id = 1')
                        ->leftJoin('accounting_season', 'accounting_season.id = accounting_income_enrolment.season_id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            if((date('H') < 15) || ($dateRestriction->allow == "Yes" && $dateRestriction->open_type == "Income"))
            {
                $incomeCodes = IncomeCode::find()->select(['id, concat(name," - ",description) as name'])->where(['income_type_id' => '1'])->all();
                $incomeCodes = ArrayHelper::map($incomeCodes, 'id', 'name');
            }else{
                $incomeCodes = IncomeCode::find()->select(['id, concat(name," - ",description) as name'])->where(['income_type_id' => '1'])->all();
                //$incomeCodes = IncomeCode::find()->select(['id, concat(name," - ",description) as name'])->where(['income_type_id' => '1'])->andWhere(['<>', 'name', 'BAL'])->all();
                $incomeCodes = ArrayHelper::map($incomeCodes, 'id', 'name');
            }

            if ($model->load(Yii::$app->request->post())) {
                $postData = Yii::$app->request->post();
                $model->or_no = $this->takeOr($model->season_id);
                if($model->or_no == 'No Available OR')
                {
    ...
    /**
     * Updates an existing IncomeEnrolment model.
    ...
    public function actionUpdate($id)
    {
        ...
        $incomeModel = Income::findOne(['income_id' => $model->id, 'income_type_id' => '1']);
        $dateRestriction = DateRestriction::findOne(['branch_id' => Yii::$app->user->identity->userinfo->BRANCH_C]);
    ...
    /**
     * Deletes an existing IncomeEnrolment model.
    ...
    public function actionDelete($id)
    {
        $model = $this->findModel($id);
        $incomeModel = Income::findOne(['income_id' => $model->id, 'income_type_id' => '1']);
        $seasonOrModel = SeasonOr::findOne(['or_no' => $model->or_no]);
    ...
    /**
     * Finds the IncomeEnrolment model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return IncomeEnrolment the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = IncomeEnrolment::findOne($id)) !== null) {
            return $model;
        }
