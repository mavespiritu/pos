...
use common\modules\accounting\models\BankDepositSearch;
...
use kartik\mpdf\Pdf;
/**
 * BankDepositController implements the CRUD actions for BankDeposit model.
 */
class BankDepositController extends Controller
{
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index'],
                        'allow' => true,
                        'roles' => ['manageBankDeposit'],
                    ],
                    [
                        'actions' => ['update'],
                        'allow' => true,
                        'roles' => ['updateBankDeposit'],
                    ],
                    [
                        'actions' => ['delete'],
                        'allow' => true,
                        'roles' => ['deleteBankDeposit'],
                    ],
                ],
            ],
        ];
    ...
    public function actionSearch()
    {
        ...
        $model->scenario = 'searchBankDeposit';
        ...
        {
            $postData = Yii::$app->request->post()['BankDeposit'];
            ...
            if($season)
            {
                ...
                if($postData['frequency_id']!="" && $postData['date_id']!="")
                {
                    ...
                    {
                        $data = $access ? $access->branch_program_id!= '' ? BankDeposit::find()
                                ->select([
                    ...
                    $content = $this->renderPartial('_bank-deposit-report', [
                        'data' => $data,
                        'season' => $season,
                        'postData' => $postData,
                        'dates' => $dates,
                    ...
                    'filename' => $postData['frequency_id'].' Report: Bank Deposit - '.$season->seasonName.' - Page '.($postData['page_id']+1).' of '.$pages.'.pdf',
                    'content' => $content,  
                    'marginLeft' => 11.4,
                    'marginRight' => 11.4,
                    ...
                    'methods' => [ 
                        'SetHeader'=>[$postData['frequency_id'].' Report: Bank Deposit - '.$season->seasonName.' - Page '.($postData['page_id']+1).' of '.$pages], 
        ...
        return $this->renderAjax('_search-bank-deposit',[
            'model' => $model,
            'seasons' => $seasons,
    ...
    public function actionPageList($season_id = '', $frequency_id = '', $date_id = '') {
        ...
        if($season)
        {
            ...
            if($frequency_id != '' && $date_id != '')
            {
                ...
                {
                    $count = $access ? $access->branch_program_id!= '' ? BankDeposit::find()
                            ->leftJoin('accounting_expense', 'accounting_expense.expense_id = accounting_expense_bank_deposit.id and accounting_expense.expense_type_id = 4')
                            ->leftJoin('accounting_season','accounting_season.id = accounting_expense.season_id')
                            ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
                            ->andWhere(['like','accounting_expense.datetime', $dates['year']])
    ...
    public function actionIndex()
    {
        ...
        $searchModel = new BankDepositSearch();
        ...
        if(in_array('TopManagement',$rolenames)){
            $overall = $access ? $access->branch_program_id!= '' ? BankDeposit::find()
            ...
            $overallCash = $access ? $access->branch_program_id!= '' ? BankDeposit::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_bank_deposit.id and accounting_expense.expense_type_id = 4')
                        ->leftJoin('accounting_season','accounting_expense.season_id = accounting_season.id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallCheck = $access ? $access->branch_program_id!= '' ? BankDeposit::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_bank_deposit.id and accounting_expense.expense_type_id = 4')
                        ->leftJoin('accounting_season','accounting_expense.season_id = accounting_season.id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallToday = $access ? $access->branch_program_id!= '' ? BankDeposit::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_bank_deposit.id and accounting_expense.expense_type_id = 4')
                        ->leftJoin('accounting_season','accounting_expense.season_id = accounting_season.id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallCashToday = $access ? $access->branch_program_id!= '' ? BankDeposit::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_bank_deposit.id and accounting_expense.expense_type_id = 4')
                        ->leftJoin('accounting_season','accounting_expense.season_id = accounting_season.id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallCheckToday = $access ? $access->branch_program_id!= '' ? BankDeposit::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_bank_deposit.id and accounting_expense.expense_type_id = 4')
                        ->leftJoin('accounting_season','accounting_expense.season_id = accounting_season.id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                {   
                   ...
                   if($selectedSeason)
                   {
                        $expenseModel->expense_type_id = 4; 
                        ...
                        \Yii::$app->getSession()->setFlash('success', 'Bank Deposit has been saved.');
    ...
    public function actionUpdate($id)
    {
        ...
        $expenseModel = Expense::findOne(['expense_id' => $model->id, 'expense_type_id' => '4']);
        $dateRestriction = DateRestriction::findOne(['branch_id' => Yii::$app->user->identity->userinfo->BRANCH_C]);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            {   
               ...
               if($selectedSeason)
               {
                    ...
                    \Yii::$app->getSession()->setFlash('success', 'Bank Deposit has been updated.');
                    return $this->redirect(['index']);
               }
            }
        }
    ...
    public function actionDelete($id)
    {
        ...
        \Yii::$app->getSession()->setFlash('success', 'Bank Deposit has been deleted.');
        return $this->redirect(['index']);
    }
