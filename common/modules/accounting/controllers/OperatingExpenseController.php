...
use common\modules\accounting\models\OperatingExpenseSearch;
...
use kartik\mpdf\Pdf;
/**
 * OperatingExpenseController implements the CRUD actions for OperatingExpense model.
 */
class OperatingExpenseController extends Controller
{
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index'],
                        'allow' => true,
                        'roles' => ['manageOperatingExpense'],
                    ],
                    [
                        'actions' => ['update'],
                        'allow' => true,
                        'roles' => ['updateOperatingExpense'],
                    ],
                    [
                        'actions' => ['delete'],
                        'allow' => true,
                        'roles' => ['deleteOperatingExpense'],
                    ],
                ],
            ],
        ];
    ...
    public function actionSearch()
    {
        ...
        $model->scenario = 'searchOperatingExpense';
        ...
        {
            $postData = Yii::$app->request->post()['OperatingExpense'];
            ...
            if($season)
            {
                ...
                if($postData['frequency_id']!="" && $postData['date_id']!="")
                {
                    ...
                    {
                        $data = $access ? $access->branch_program_id!= '' ? OperatingExpense::find()
                                ->select([
                    ...
                    $content = $this->renderPartial('_operating-expense-report', [
                        'data' => $data,
                        'season' => $season,
                        'postData' => $postData,
                        'dates' => $dates,
                    ...
                    'filename' => $postData['frequency_id'].' Report: Operating Expense - '.$season->seasonName.' - Page '.($postData['page_id']+1).' of '.$pages.'.pdf',
                    'content' => $content,  
                    'marginLeft' => 11.4,
                    'marginRight' => 11.4,
                    ...
                    'options' => ['title' => 'Operating Expense'],
                    'methods' => [ 
                        'SetHeader'=>[$postData['frequency_id'].' Report: Operating Expense - '.$season->seasonName.' - Page '.($postData['page_id']+1).' of '.$pages], 
        ...
        return $this->renderAjax('_search-operating-expense',[
            'model' => $model,
            'seasons' => $seasons,
    ...
    public function actionPageList($season_id = '', $frequency_id = '', $date_id = '') {
        ...
        if($season)
        {
            ...
            if($frequency_id != '' && $date_id != '')
            {
                ...
                {
                    $count = $access ? $access->branch_program_id!= '' ? OperatingExpense::find()
                            ->leftJoin('accounting_expense', 'accounting_expense.expense_id = accounting_expense_operating_expense.id and accounting_expense.expense_type_id = 5')
                            ->leftJoin('accounting_season','accounting_season.id = accounting_expense.season_id')
                            ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
                            ->andWhere(['like','accounting_expense.datetime', $dates['year']])
    ...
    /**
     * Lists all OperatingExpense models.
     * @return mixed
     */
    public function actionIndex()
    {
        ...
        $searchModel = new OperatingExpenseSearch();
        ...
        if(in_array('TopManagement',$rolenames)){
            $overall = $access ? $access->branch_program_id!= '' ? OperatingExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_operating_expense.id as id,  
                            sum(accounting_expense_operating_expense.staff_salary + accounting_expense_operating_expense.cash_pf + accounting_expense_operating_expense.rent + accounting_expense_operating_expense.utilities + accounting_expense_operating_expense.equipment_and_labor + accounting_expense_operating_expense.bir_and_docs + accounting_expense_operating_expense.marketing) as total 
                            ...
                            group by accounting_expense_operating_expense.id
                        )'], 't.id = accounting_expense_operating_expense.id')
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_operating_expense.id and accounting_expense.expense_type_id = 5')
            ...
            $overallCash = $access ? $access->branch_program_id!= '' ? OperatingExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_operating_expense.id as id,  
            ...
            $overallCheck = $access ? $access->branch_program_id!= '' ? OperatingExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_operating_expense.id as id,  
            ...
            $overallToday = $access ? $access->branch_program_id!= '' ? OperatingExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_operating_expense.id as id,  
            ...
            $overallCashToday = $access ? $access->branch_program_id!= '' ? OperatingExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_operating_expense.id as id,  
            ...
            $overallCheckToday = $access ? $access->branch_program_id!= '' ? OperatingExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_operating_expense.id as id,  
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                {   
                   ...
                   if($selectedSeason)
                   {
                        $expenseModel->expense_type_id = 5; 
                        ...
                        \Yii::$app->getSession()->setFlash('success', 'Operating expense has been saved.');
    ...
    /**
     * Updates an existing OperatingExpense model.
    ...
    public function actionUpdate($id)
    {
        ...
        $expenseModel = Expense::findOne(['expense_id' => $model->id, 'expense_type_id' => '5']);
        $dateRestriction = DateRestriction::findOne(['branch_id' => Yii::$app->user->identity->userinfo->BRANCH_C]);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            {   
               ...
               if($selectedSeason)
               {
                    ...
                    \Yii::$app->getSession()->setFlash('success', 'Operating expense has been updated.');
                    return $this->redirect(['index']);
               }
            }
        }
    ...
    /**
     * Deletes an existing OperatingExpense model.
    ...
    public function actionDelete($id)
    {
        ...
        \Yii::$app->getSession()->setFlash('success', 'Operating expense has been deleted.');
        return $this->redirect(['index']);
    }

    /**
     * Finds the OperatingExpense model based on its primary key value.
     * If the model is not found, a 505 HTTP exception will be thrown.
     * @param integer $id
     * @return OperatingExpense the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = OperatingExpense::findOne($id)) !== null) {
            return $model;
        }
