...
use common\modules\accounting\models\PettyExpenseSearch;
...
use kartik\mpdf\Pdf;
/**
 * PettyExpenseController implements the CRUD actions for PettyExpense model.
 */
class PettyExpenseController extends Controller
{
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index'],
                        'allow' => true,
                        'roles' => ['managePettyExpense'],
                    ],
                    [
                        'actions' => ['update'],
                        'allow' => true,
                        'roles' => ['updatePettyExpense'],
                    ],
                    [
                        'actions' => ['delete'],
                        'allow' => true,
                        'roles' => ['deletePettyExpense'],
                    ],
                ],
            ],
        ];
    ...
    {
        $model = new PettyExpense();
        $model->scenario = 'searchPettyExpense';
        ...
        {
            $postData = Yii::$app->request->post()['PettyExpense'];
            ...
            if($season)
            {
                ...
                if($postData['frequency_id']!="" && $postData['date_id']!="")
                {
                    ...
                    {
                        $data = $access ? $access->branch_program_id!= '' ? PettyExpense::find()
                                ->select([
                    ...
                    $content = $this->renderPartial('_petty-expense-report', [
                        'data' => $data,
                        'season' => $season,
                        'postData' => $postData,
                        'dates' => $dates,
                    ...
                    'filename' => $postData['frequency_id'].' Report: Petty Expense - '.$season->seasonName.' - Page '.($postData['page_id']+1).' of '.$pages.'.pdf',
                    'content' => $content,  
                    'marginLeft' => 11.4,
                    'marginRight' => 11.4,
                    ...
                    'options' => ['title' => 'Petty Expense'],
                    'methods' => [ 
                        'SetHeader'=>[$postData['frequency_id'].' Report: Petty Expense - '.$season->seasonName.' - Page '.($postData['page_id']+1).' of '.$pages], 
        ...
        return $this->renderAjax('_search-petty-expense',[
            'model' => $model,
            'seasons' => $seasons,
    ...
    public function actionPageList($season_id = '', $frequency_id = '', $date_id = '') {
        ...
        if($season)
        {
            ...
            if($frequency_id != '' && $date_id != '')
            {
                ...
                {
                    $count = $access ? $access->branch_program_id!= '' ? PettyExpense::find()
                            ->leftJoin('accounting_expense', 'accounting_expense.expense_id = accounting_expense_petty_expense.id and accounting_expense.expense_type_id = 1')
                            ->leftJoin('accounting_season','accounting_season.id = accounting_expense.season_id')
                            ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
                            ->andWhere(['like','accounting_expense.datetime', $dates['year']])
    ...
    public function actionIndex()
    {
        ...
        $searchModel = new PettyExpenseSearch();
        ...
        if(in_array('TopManagement',$rolenames)){
            $overall = $access ? $access->branch_program_id!= '' ? PettyExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_petty_expense.id as id,  
                            ...
                            group by accounting_expense_petty_expense.id
                        )'], 't.id = accounting_expense_petty_expense.id')
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_petty_expense.id and accounting_expense.expense_type_id = 1')
            ...
            $overallCash = $access ? $access->branch_program_id!= '' ? PettyExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_petty_expense.id as id,  
            ...
            $overallCheck = $access ? $access->branch_program_id!= '' ? PettyExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_petty_expense.id as id,  
            ...
            $overallToday = $access ? $access->branch_program_id!= '' ? PettyExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_petty_expense.id as id,  
            ...
            $overallCashToday = $access ? $access->branch_program_id!= '' ? PettyExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_petty_expense.id as id,  
            ...
            $overallCheckToday = $access ? $access->branch_program_id!= '' ? PettyExpense::find()
                        ->select(['sum(t.total) as total'])
                        ->leftJoin(['t' => '(
                            select 
                            accounting_expense_petty_expense.id as id,  
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                {   
                   ...
                   if($selectedSeason)
                   {
                        $expenseModel->expense_type_id = 1; 
                        ...
                        \Yii::$app->getSession()->setFlash('success', 'Petty expense has been saved.');
    ...
    /**
     * Updates an existing PettyExpense model.
    ...
    public function actionUpdate($id)
    {
        ...
        $dateRestriction = DateRestriction::findOne(['branch_id' => Yii::$app->user->identity->userinfo->BRANCH_C]);
        $expenseModel = Expense::findOne(['expense_id' => $model->id, 'expense_type_id' => '1']);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            {   
               ...
               if($selectedSeason)
               {
                    ...
                    \Yii::$app->getSession()->setFlash('success', 'Petty expense has been updated.');
                    return $this->redirect(['index']);
               }
            }
        }
    ...
    /**
     * Deletes an existing PettyExpense model.
    ...
    /**
     * Finds the PettyExpense model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return PettyExpense the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = PettyExpense::findOne($id)) !== null) {
            return $model;
        }
