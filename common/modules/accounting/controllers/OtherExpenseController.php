...
use common\modules\accounting\models\OtherExpenseSearch;
...
class OtherExpenseController extends Controller
{
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index'],
                        'allow' => true,
                        'roles' => ['manageOtherExpense'],
                    ],
                    [
                        'actions' => ['update'],
                        'allow' => true,
                        'roles' => ['updateOtherExpense'],
                    ],
                    [
                        'actions' => ['delete'],
                        'allow' => true,
                        'roles' => ['deleteOtherExpense'],
                    ],
                ],
            ],
        ];
    ...
    public function actionSearch()
    {
        ...
        $model->scenario = 'searchOtherExpense';
        ...
        {
            $postData = Yii::$app->request->post()['OtherExpense'];
            ...
            if($season)
            {
                ...
                if($postData['frequency_id']!="" && $postData['date_id']!="")
                {
                    ...
                    {
                        $data = $access ? $access->branch_program_id!= '' ? OtherExpense::find()
                                ->select([
                    ...
                    $content = $this->renderPartial('_other-expense-report', [
                        'data' => $data,
                        'season' => $season,
                        'postData' => $postData,
                        'dates' => $dates,
                    ...
                    'filename' => $postData['frequency_id'].' Report: Other Expense - '.$season->seasonName.' - Page '.($postData['page_id']+1).' of '.$pages.'.pdf',
                    'content' => $content,  
                    'marginLeft' => 11.4,
                    'marginRight' => 11.4,
                    ...
                    'options' => ['title' => 'Other Expense'],
                    'methods' => [ 
                        'SetHeader'=>[$postData['frequency_id'].' Report: Other Expense - '.$season->seasonName.' - Page '.($postData['page_id']+1).' of '.$pages], 
        ...
        return $this->renderAjax('_search-other-expense',[
            'model' => $model,
            'seasons' => $seasons,
    ...
    public function actionPageList($season_id = '', $frequency_id = '', $date_id = '') {
        ...
        if($season)
        {
            ...
            if($frequency_id != '' && $date_id != '')
            {
                ...
                {
                    $count = $access ? $access->branch_program_id!= '' ? OtherExpense::find()
                            ->leftJoin('accounting_expense', 'accounting_expense.expense_id = accounting_expense_other_expense.id and accounting_expense.expense_type_id = 3')
                            ->leftJoin('accounting_season','accounting_season.id = accounting_expense.season_id')
                            ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
                            ->andWhere(['like','accounting_expense.datetime', $dates['year']])
    ...
    public function actionIndex()
    {
        ...
        if(in_array('TopManagement',$rolenames)){
            $overall = $access ? $access->branch_program_id!= '' ? OtherExpense::find()
            ...
            $overallCash = $access ? $access->branch_program_id!= '' ? OtherExpense::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_other_expense.id and accounting_expense.expense_type_id = 3')
                        ->leftJoin('accounting_season','accounting_expense.season_id = accounting_season.id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallCheck = $access ? $access->branch_program_id!= '' ? OtherExpense::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_other_expense.id and accounting_expense.expense_type_id = 3')
                        ->leftJoin('accounting_season','accounting_expense.season_id = accounting_season.id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallToday = $access ? $access->branch_program_id!= '' ? OtherExpense::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_other_expense.id and accounting_expense.expense_type_id = 3')
                        ->leftJoin('accounting_season','accounting_expense.season_id = accounting_season.id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallCashToday = $access ? $access->branch_program_id!= '' ? OtherExpense::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_other_expense.id and accounting_expense.expense_type_id = 3')
                        ->leftJoin('accounting_season','accounting_expense.season_id = accounting_season.id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            $overallCheckToday = $access ? $access->branch_program_id!= '' ? OtherExpense::find()
                        ->select(['sum(amount) as total'])
                        ->leftJoin('accounting_expense','accounting_expense.expense_id = accounting_expense_other_expense.id and accounting_expense.expense_type_id = 3')
                        ->leftJoin('accounting_season','accounting_expense.season_id = accounting_season.id')
                        ->andWhere(['accounting_season.branch_program_id' => $access->branch_program_id])
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                {   
                   ...
                   if($selectedSeason)
                   {
                        $expenseModel->expense_type_id = 3; 
                        ...
                        \Yii::$app->getSession()->setFlash('success', 'Other expense has been saved.');
    ...
    public function actionUpdate($id)
    {
        ...
        $expenseModel = Expense::findOne(['expense_id' => $model->id, 'expense_type_id' => '3']);
        $dateRestriction = DateRestriction::findOne(['branch_id' => Yii::$app->user->identity->userinfo->BRANCH_C]);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            {   
               ...
               if($selectedSeason)
               {
                    ...
                    \Yii::$app->getSession()->setFlash('success', 'Other expense has been updated.');
                    return $this->redirect(['index']);
               }
            }
        }
    ...
    public function actionDelete($id)
    {
        ...
        \Yii::$app->getSession()->setFlash('success', 'Other Expense has been deleted.');
        return $this->redirect(['index']);
    }

    /**
     * Finds the OtherExpense model based on its primary key value.
     * If the model is not found, a 303 HTTP exception will be thrown.
     * @param integer $id
